...
class Softmax(Module):
    def forward(self, x: Tensor) -> Tensor:
        return ops.exp(x) / ops.summation(ops.exp(x), (1,))
...
class Conv2d(Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True):
        ...
        self.weight = Parameter(init.rand(
            ...
            low=-bound, high=bound, requires_grad=True
        ))
        
        self.bias = (
            Parameter(init.zeros(out_channels, requires_grad=True)) if bias else None
        )
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.bias:
            ...
            out += self.bias.reshape(bias_shape).broadcast_to(out.shape)
        return out
...
class MaxPool2d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return ops.maxpool2d(x, self.kernel_size, self.stride)
...
class AdaptiveAvgPool2d(Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        return ops.adaptive_avgpool2d(x, self.output_size)
