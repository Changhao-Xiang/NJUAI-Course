...
class TsfKNN(MLForecastModel):
    ...
    def _search_decompose(self, x, X_s, seq_len, pred_len):
        ...
        elif "stl" in self.decomposition:
            x_trend, x_seasonal, x_residual = STL_decomposition(x, seasonal_period=24)
            X_s_trend, X_s_seasonal, X_s_residual = STL_decomposition(X_s, seasonal_period=24)
        elif "x11" in self.decomposition:
            x_trend, x_seasonal, x_residual = X11_decomposition(x, seasonal_period=24)
            X_s_trend, X_s_seasonal, X_s_residual = X11_decomposition(X_s, seasonal_period=24)
    ...
    def _forecast(self, X: np.ndarray, pred_len) -> np.ndarray:
        ...
        for i in trange(X.shape[0]):
            x = X[i, :, :]
            if "no" not in self.decomposition:
