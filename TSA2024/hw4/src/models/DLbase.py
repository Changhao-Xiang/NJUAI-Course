...
class DLForecastModel(MLForecastModel):
    ...
    def _fit(self, train_X: np.ndarray, val_X=None):
        train_loader = DataLoader(
            DLDataset(train_X, self.args.seq_len, self.args.pred_len),
            batch_size=self.args.batch_size,
            shuffle=True,
        )
        val_loader = DataLoader(
            DLDataset(val_X, self.args.seq_len, self.args.pred_len),
            batch_size=self.args.batch_size,
            shuffle=False,
        )

        dataset = self.args.data_path.split("/")[-1]
        dataset = dataset.split(".")[0]
        path = os.path.join(
            "checkpoints", f"{self.args.model}_{self.args.e_layers}_{dataset}_{self.args.pred_len}"
        )
        ...
        for epoch in range(train_epochs):
            ...
            print(
                "Epoch: {} cost time: {} Train Loss: {} Val Loss: {}".format(
    ...
    def _forecast(self, X: np.ndarray, pred_len) -> np.ndarray:
        test_loader = DataLoader(
            DLDataset(X, self.args.seq_len, self.args.pred_len, mode="predict"),
            batch_size=self.args.batch_size,
            shuffle=False,
        )
        # predict
