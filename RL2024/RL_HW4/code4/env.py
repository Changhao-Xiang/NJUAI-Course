...
class Make_Env(object):
    def __init__(self, env_mode=2):
        ...
        for pos in self.blocks:
            ...
            self.image_init[
                ...
                (self.margin + y * self.grid_size) : (self.margin + (y + 1) * self.grid_size),
            ] = self.image_wall
    ...
    def get_ob(self, state=-1):
        ...
        observ[
            :,
            (self.margin + self.pos_man[0] * self.grid_size) : (
                self.margin + (self.pos_man[0] + 1) * self.grid_size
            ),
            (self.margin + self.pos_man[1] * self.grid_size) : (
                self.margin + (self.pos_man[1] + 1) * self.grid_size
            ),
        ] = self.image_man
        if self.use_key:
            observ[
                :,
                (self.margin + self.pos_key[0] * self.grid_size) : (
                    self.margin + (self.pos_key[0] + 1) * self.grid_size
                ),
                (self.margin + self.pos_key[1] * self.grid_size) : (
                    self.margin + (self.pos_key[1] + 1) * self.grid_size
                ),
            ] = self.image_key
        observ[
            :,
            (self.margin + self.pos_door[0] * self.grid_size) : (
                self.margin + (self.pos_door[0] + 1) * self.grid_size
            ),
            (self.margin + self.pos_door[1] * self.grid_size) : (
                self.margin + (self.pos_door[1] + 1) * self.grid_size
            ),
        ] = self.image_door
