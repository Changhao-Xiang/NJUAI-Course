...
class TD3:
    def __init__(self, args):
        ...
        self.actor_eval = Deterministic_Actor(
            self.state_dim, self.action_dim, self.hidden_dim, self.action_clip
        ).to(self.device)
    ...
    def compute_target_q(self, state, action, next_state, reward, done):
        with torch.no_grad():
            ...
            next_action = (self.actor_target(next_state) + noise).clamp(-self.action_clip, self.action_clip)
