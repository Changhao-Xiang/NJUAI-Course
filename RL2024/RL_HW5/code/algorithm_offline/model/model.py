...
class Ensemble_Guassian_Dynamics:
    def __init__(self, args):
        ...
        self.ensemble_dynamics = [
            Stochastic_Dynamics(
                self.state_dim, self.action_dim, self.hidden_dim, self.normalizer, self.state_clip
            ).to(self.device)
            for _ in range(self.num_model)
        ]
        self.ensemble_rewards = [
            Reward(self.state_dim, self.action_dim, self.hidden_dim).to(self.device)
            for _ in range(self.num_model)
        ]

        self.optim_dynamics = [
            torch.optim.Adam(self.ensemble_dynamics[i].parameters(), lr=self.lr)
            for i in range(self.num_model)
        ]
        self.optim_rewards = [
            torch.optim.Adam(self.ensemble_rewards[i].parameters(), lr=self.lr) for i in range(self.num_model)
        ]
            ...
            for i in range(self.num_epoch):
                ...
                for j in range(batch_size // self.batch_size):
                    ...
                    dynamics_loss = torch.mean(
                        torch.pow(batch_next_state - pre_next_state, 2) * (1.0 - batch_done)
                    )
                    reward_loss = torch.mean(torch.pow(batch_reward - pre_reward, 2) * (1.0 - batch_done))
    ...
    def save_model(self, path):
        ...
        state_dict = {
            "ensemble_dynamics": dynamics_state_dict,
