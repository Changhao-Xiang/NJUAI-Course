...
class RolloutStorage(object):
    ...
    def add(self, obs, actions, rewards, next_obs, masks):
        ...
        self.next_obs[self.step].copy_(
            torch.tensor(next_obs[None, :], dtype=torch.uint8).squeeze(0).squeeze(0)
        )
    ...
    def sample(self, mini_batch_size=None):
        indices = np.random.randint(0, self.current_size, mini_batch_size).tolist()
